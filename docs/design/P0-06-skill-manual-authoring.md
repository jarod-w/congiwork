...
**权限一次性预检**（SK-7）是重要的体验设计：如果在执行到第 4 步时才问「要不要授权发邮件」，用户已经等了一分钟，体验很差且容易放弃。预检把所有权限问题前置到执行前的一张卡片。
...
### 5.5 预置示例 Skill（按 A1 = 市场 / 运营 落实）

§8 风险表第一行的对策要求「提供 3–5 个预置示例 Skill 可一键复制修改」。内容如下，全部锚定 A1 的三类高频任务：

| # | 名称 | 步骤类型序列 | 所需 Scope | 示范什么 |
|---|---|---|---|---|
| 1 | 渠道数据周报 | `collect_input` → `llm` → `approval` → `llm`(导出) | **零** | 最短闭环。上传表格即可跑通，是新用户的第一个 Skill |
| 2 | 用户反馈分类打标 | `collect_input` → `llm` → `approval` | **零** | 批量处理 + 分类口径可在 instruction 里改 |
| 3 | 内容多平台改写 | `collect_input` → `llm` → `llm` | **零** | 一次输入多份产出；示范 `input_schema` 的枚举参数（选平台） |
| 4 | 季度渠道复盘 | `collect_input` → `tool`(读数) → `llm` → `approval` → `tool`(发送) | `tool:*:read` + 发送类 | 唯一含 `irreversible` 步骤的示例，**专门用来让用户第一次看清审批长什么样** |
| 5 | 月度报告草稿 | `llm`(读上月产物 + 记忆) → `approval` | **零** | 示范记忆复用：这个 Skill 第二次跑才显出价值 |

四条设计约束：

1. **五个里有四个零授权。** 预置示例的作用是降低「不知道该创建什么」的门槛（§8 首行风险），如果一打开就要求连接工具，它反而变成授权劝说页。示例 4 保留发送步骤是刻意的——它是审批体验的教学样本，且默认不可直接运行，必须先补授权。
2. **一键复制而非直接可用。** 点「用这个」= 复制一份到用户自己的库，`source = 'preset_copy'`、`status = 'draft'`，用户至少确认一次参数与步骤才能转 `active`。预置示例本身不属于任何用户，不可被执行。
3. **`source = 'preset_copy'` 不计入 Phase 1 退出条件。** 退出条件是「用户**自发**创建 ≥3 个 Skill 并复用」，把复制预置示例算进去等于自己给自己刷指标。埋点口径见 `P0-04` §9。若用户把复制来的 Skill 改到面目全非，仍不改 `source`——想统计「被改造后的复用」另立指标，不污染退出条件。
4. **示例 4 的发送步骤不写死具体工具——这条在 2026-08-18 改定后不但保留，理由还更强了。** 改定前的理由是「Gmail 不在首批，邮件只能走桌面 MailAdapter」；改定后 Gmail 回到首批（`P0-05` §2.1），但**它能否上线取决于 Google CASA 审核，到 G2 检查点才有结论**（`P0-05` §2.1.1）。也就是说这个示例的发送工具在 Phase 1 有三种可能：Gmail 连接器、桌面 MailAdapter、或只剩草稿 + 用户自己发。
   因此该步骤仍声明为「发送给相关人」的意图 + `needs_clarification`，用户复制后由权限预检（SK-7）填入**当时实际可用**的发送工具。**预置示例的定义文件里不得出现具体连接器名**——否则 CASA 一旦不通过，就得改预置内容重新发版。
...
步骤
  1. ▸ 从 Notion 数据库拉取该季度各渠道转化数据  🔧 Notion  🔓 读取
...
- **步骤 4 的工具显示为「待确定」是刻意的，不是示意图没画完。** 它对应 §5.5 约束 4：发送类步骤在复制时由权限预检填入当时实际可用的工具（Gmail 连接器 / 桌面 MailAdapter / 无）。编辑器必须能渲染「工具未定」这个状态，而不是强制用户先选一个工具才能保存草稿。
...
| 风险 | 影响 | 对策 |
|---|---|---|
| 用户不知道该创建什么 Skill，创建量为 0（Phase 1 退出条件不达标） | 极高 | 主动识别：同一用户 7 天内做过 ≥2 次相似任务时，在结果卡片主动建议「要不要存成技能」；提供 5 个预置示例 Skill 可一键复制修改（**内容见 §5.5**）；另以 `P0-01` 的 `recurring_deliverables` 作为推荐种子——用户自己说过「每周要交渠道周报」，就直接建议把它做成 Skill |
...
| 用户改了外部系统（如 Notion 数据库改名、页面被删），Skill 静默失效 | 中 | `on_error: ask_user` 作为工具步骤的推荐默认；失败后在 Library 中标红并给出修复建议 |
...
6. Phase 1 退出条件支撑：能统计「自发创建 ≥3 个 Skill 并复用」的用户数，且统计**排除** `source = 'preset_copy'`（自动化测试断言：复制 3 个预置示例的用户不被计入达标用户）。
...
1. ~~预置示例 Skill 的内容依赖 §10.1 目标用户定位~~ → **已决（A1 = 市场 / 运营）**：五个示例见 §5.5。
2. ~~Skill 嵌套调用（`type: skill`）Phase 1 是否真的需要？~~ → **已决（B8，2026-08-18 确认默认做法）**：**做，但限 1 层。**
   - 限 1 层是硬约束不是建议：**执行器必须在运行时拒绝第 2 层嵌套并给出可读错误**，不能靠「文档里说了限 1 层」。没有运行时检查的深度限制等于没有限制。
   - **本项仍是 A9.1 触发式预案的第三刀**（不做省 3d）。确认「做」意味着这 3 天留在预算内；若第 3 周末容量复核落后，它是可被撤回的。**因此实现上不要让其他功能依赖嵌套能力**——被砍时不应产生连锁返工。
