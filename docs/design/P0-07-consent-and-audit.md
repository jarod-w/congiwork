...
CREATE TABLE execution_audit (
  ...
  action        text NOT NULL,              -- 'gmail.send_message'
...
2. **市场边界。** 当前模型面向不做欧盟市场的场景。若未来进入欧盟（GDPR）或中国大陆（PIPL 对员工个人信息、尤其敏感信息有独立于「同意」之外的企业合规义务），本节需重新评估，不可直接套用。这一条要写进产品的地域开放决策检查项。
...
| 项 | 内容 |
|---|---|
| 样本 | 8–10 人，来自 §10.1 目标用户群（销售/运营/市场），非熟人 |
| 材料 | 真实的授权卡片原型（§6.1 四段式），覆盖 3 个代表性 Scope：`tool:gmail:read`（L2）、`desktop:excel:automate`（L3）、`telemetry:*:collect`（L4） |
...
2. §8.3 零授权 E2E 测试通过。
3. 每个 Scope 的授权卡片四段齐全，由产品逐项签字确认。
...
1. `consent_record` 永久保留 vs 账号删除后匿名化保留——需法务确认在目标市场是否成立。当前设计倾向匿名化保留，需确认。
2. 「关闭授权时是否连带删除已产生数据」默认值：当前设计为默认不删（让用户选）。另一种做法是默认删除、可选保留。倾向前者（避免用户误删），但需在 §11 实验中一并测试用户预期。
